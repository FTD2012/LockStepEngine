...
public abstract class GLogger : MonoBehaviour
{
    ...
    /// <summary>
    /// The black color's Hexadecimal code
    /// </summary>
    /// <returns> The hex code string </returns>
    protected string blackHEX => "#000000";
    ...
    protected virtual void DoLog(string message, string color = "#FFFFFF", Object sender = null)
    {
        ...
        if (sender != null)
        {
            Debug.Log($"<color={color}> [{Time.frameCount}] {message} </color>", sender);

        }
        else
        {
            Debug.Log($"<color={color}> [{Time.frameCount}]  {message} </color>");
        }
    }
